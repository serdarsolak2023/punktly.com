...
import { days, weeklyGoals, dailyGoals, rareBadges, initialChildren, initialTasks, initialRewards, initialShop, initialChests, extraShopItems, 
  punktlyCoinPositions, profileBadgeOptions, taskPresets, taskPacks, legalPages } from "./components/appdata";
...
const themeClass =
  child.theme === "🦁 Löwe"
    ...
    : child.theme === "🐼 Panda"
    ? "from-slate-100 to-gray-300"
    : child.theme === "🦜 Papagei"
    ? "from-green-100 to-yellow-200"
    : child.theme === "🐧 Pinguin"
...
async function resetPasswordWithEmail() {
  if (!emailLogin.trim()) {
    celebrate("Bitte erst deine E-Mail eingeben.");
    return;
  }

  try {
    await sendPasswordResetEmail(auth, emailLogin.trim());

    playSound("success");
    celebrate("Passwort-Link wurde gesendet. Bitte prüfe deine E-Mail und den Spam-Ordner.");
  } catch (error: any) {
    console.error(error);
    celebrate(error?.message || "Passwort zurücksetzen fehlgeschlagen.");
  }
}
...
/>
      ...
      <p className="text-xs font-bold text-slate-500">
        Falls keine E-Mail ankommt, prüfe bitte auch deinen Spam-Ordner.
      </p>
    </div>
  )}
</div>
  ...
  <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
    {["alle", "wartet", "offen", "erledigt", "verpasst"].map((status) => (
      <button
        ...
        className={`rounded-full px-4 py-2 text-sm font-black text-center ${
